    ...
    for limit in limits[1:]:
        ...
        for existing_limit in merged_limits:
            if limit[0] >= existing_limit[0] and limit[1] <= existing_limit[1]:
                merged = True
            elif (limit[0] >= existing_limit[0] and limit[0] <= existing_limit[1]) and limit[1] > existing_limit[1]:
                existing_limit[1] = limit[1]  # on étend à droite
                merged = True
            elif limit[0] < existing_limit[0] and (limit[1] <= existing_limit[1] and limit[1] >= existing_limit[0]):
                existing_limit[0] = limit[0]
                merged = True  # on étend à gauche
        if not merged:  # disjoint
...
if __name__ == '__main__':
    ...
    for cur_y in tqdm(range(y_min, y_max+1)):
        covered_x = []
        for sensor in sensors:
            x, y = sensor.coords()
            diff_y = abs(cur_y - y)
        ...
        covered_x.sort(key=lambda i: i[0])
        #print(f"{cur_y}: {covered_x}")
        ranges = merge(covered_x)
        if len(ranges) > 1 or ranges[0][0] >= 0 or ranges[0][1] <= x_max:
            print(f"{cur_y}: {ranges}")
