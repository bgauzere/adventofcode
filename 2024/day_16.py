...
def shortest_path(lab, start, end):
    distances = {(node,dir) : float("inf") for node in lab.nodes for dir in range(4)}
    distances[(start,0)] = 0
    ...
    while priority_queue:
        ...
        for next_node in lab.neighbors(cur_node):
            next_dir = lab[cur_node][next_node]["dir"]  # Get the direction of the next edge
            turn_cost = diff_dir(cur_dir, next_dir) * 1000  # Compute the cost of turning
            new_distance = cur_dist + turn_cost + 1  # Add movement cost
            if new_distance < distances[next_node,next_dir]:  # Update if we found a shorter path
                ...
                heapq.heappush(priority_queue, (new_distance, next_node, next_dir))  # Push the updated direction
    return distances, map_directions, pred
